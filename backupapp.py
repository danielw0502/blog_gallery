        ...
        if not (entry.title and entry.content):
            ...
            try:
                with database.atomic():
                    for ii in entry.tag_name.strip('|').split('|'):
                        if ii in tag_dict:
                            tag_dict[ii] += 1
                        else:
                            tag_dict[ii] = 1
                        print ii, tag_dict[ii]
                    entry.save()
...
def find_blog_with_tag(tag):
    ...
    return object_list('index.html', python_entries, t = tag_dict, check_bounds=False)

@app.route('/archives')
def distinct_tag():
    query_tag = (Entry.select().distinct())
    #tag = (Entry.get_tag())
    return object_list('sidebar.html', query_tag, check_bounds=False)

@app.route('/drafts/')
@login_required
def drafts():
    query = Entry.drafts().order_by(Entry.timestamp.desc())
    return object_list('index.html', query, t = tag_dict, check_bounds=False)
