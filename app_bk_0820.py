...
class NewAlbumForm(Form):
    ...
    photo = FileField(u'图片', validators=[FileRequired(u'你还没有选择图片！'), FileAllowed(photos, u'只能上传图片！')])
    submit = SubmitField(u'提交')

class AddPhotoForm(Form):
    photo = FileField(u'图片', validators=[FileRequired(),FileAllowed(photos, u'只能上传图片！')])
    submit = SubmitField(u'提交')
#上传表单end
...
def image_resize(image, base_width):
    filename, ext = os.path.splittext(image)
...
def gallery():
    albums = Album.select()
    return object_list('gallery.html', albums)
...
def album(id):
    ...
    pagination = album.photos.order_by(Photo.order.asc()).paginate(page, 5, error_out=False)
...
def create_gallery():
    ...
    if form.validate_on_submit():
        ...
        for url in images:
            photo = Photo(url=url[0], url_s = url[1], url_t = url[2], album = album)
            photo.save()
...
def photo(id):
    photo = Photo.select().where(Photo.id = id)
    ...
    page = request.args.get('page', 1, type=int)
    return render_template('photo.html', album=album, photo=photo, pagination=pagination, photo_index=photo_index, photo_sum=photo_sum)
    

@app.route('/photo/p/<int:id>')
def photo_previous(id):
    "redirect to previous image"
    photo_now = Photo.select().where(Photo.id = id)
